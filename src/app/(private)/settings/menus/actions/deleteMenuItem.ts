...
export async function deleteMenuItem(formData: FormData) {
  try {
    ...
    if (existingItem.children && existingItem.children.length > 0) {
      throw new Error(
        'Não é possível excluir um item que possui subitens. Remova os subitens primeiro.'
      );
    }
    ...
    await logAudit(user.id, 'menu.delete', `Item de menu "${existingItem.label}" excluído`, {
      item: existingItem,
    });
