    ...
    if (resultado.id) {
      ...
      createdPaymentId = await prisma.$transaction(async (tx) => {
        ...
        const order = await tx.order.findUnique({
          ...
          select: { status: true, orderItems: { select: { productId: true, isOrderBump: true } } }, // Select only needed fields
        });
        ...
        const newItems = dados.items.filter(
          (item) => {
            // Verificamos se o item já existe no pedido
            if (existingItemIds.includes(item.id)) return false;
            
            // Encontramos o item principal (não order bump) no pedido
            const mainOrderItem = order.orderItems.find(oi => oi.isOrderBump === false);
            
            // Ignorar o item principal (que não é order bump)
            return item.id !== mainOrderItem?.productId;
          }
        );
        ...
        const paymentRecord = await tx.payment.create({
          data: {
            orderId: dados.orderId,
            status: resultado.status ? resultado.status.toUpperCase() : 'PENDING',
            method: 'pix',
            ...
            rawData: pixSimplificado ? { json: pixSimplificado } : undefined,
          },
        });
